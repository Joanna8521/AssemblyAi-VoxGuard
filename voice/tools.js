...
export function systemPrompt(corpus) {
  ...
  return [
    ...
    ``,
    `People pause mid-thought, so one instruction usually arrives as two or three`,
    `turns. "Watch my daily takings and tell me the moment they fall off a cliff."`,
    `[pause] "Never touch prices and never spend money on ads." is one instruction`,
    `split by a breath, not a description followed by an unrelated rule.`,
    ``,
    `Hold what came before. If work was described in an earlier turn and no`,
    `mission is open, the next turn completes it: call start_mission with the`,
    `work from the first turn and the limits from the second. Recording only the`,
    `prohibitions leaves somebody who asked for something watched with nothing`,
    `watching it, and the boundaries they set apply to no work at all.`,
    ``,
    `Never let a turn that describes work pass without opening a mission for it.`,
    `If they seem to be still speaking, open it on what you have; a mission is`,
    `amended by the next sentence, and an unopened one is not.`,
    ``,
    ...
    ``,
    `When they are only stating rules, call compile_policy or amend_policy`,
    `instead. This holds whether or not a mission is open: somebody who opens`,
    `with "never touch prices and never spend money on ads" has told you two`,
    `things worth keeping, and waiting for a situation before writing them down`,
    `loses them. Record first, and the next thing they say has boundaries around`,
    `it already.`,
    ``,
